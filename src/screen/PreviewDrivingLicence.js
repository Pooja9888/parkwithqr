...
const PreviewDrivingLicence = ({ route,navigation }) => {
  const { doc } = route.params || {};  
  const handleEdit = () => {
    navigation.navigate('EditDrivingLicenceForm', { doc });
  };
    ...
    if (route?.params?.fromForm) {
      try {
        ...
        if (response?.status === 200) {
          Alert.alert("Driving submitted successfully!");
          navigation.navigate(genericEnum.dashboard);
        }
      } catch (err) {
        console.log(err);
...
export default PreviewDrivingLicence;
